    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Track timelines, assignments, and deliverables.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {PROJECTS.map((project) => (
          <div key={project.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm hover:shadow-md transition-all group flex flex-col justify-between h-full">
            <div>
              ...
              <h3 className="font-bold text-slate-900 dark:text-white text-lg mb-2 line-clamp-1">{project.name}</h3>
            ...
            <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-4 mt-auto">
              <div className="flex -space-x-2.5">
                ...
                <div className="w-7 h-7 rounded-full border-2 border-white dark:border-slate-900 bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[10px] font-medium text-slate-500 dark:text-slate-400">
                  +1
                </div>
              </div>
        ...
        <button className="flex flex-col items-center justify-center gap-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700 p-6 hover:border-nexus-400 dark:hover:border-nexus-600 hover:bg-nexus-50 dark:hover:bg-nexus-900/10 transition-all group min-h-[280px]">
           <div className="w-12 h-12 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400 group-hover:text-nexus-600 group-hover:border-nexus-200 transition-colors">
              <Plus className="w-6 h-6" />
           </div>
           <span className="font-medium text-slate-500 dark:text-slate-400 group-hover:text-nexus-700 dark:group-hover:text-nexus-300">Create New Project</span>
        </button>
      </div>
    </div>
  );
